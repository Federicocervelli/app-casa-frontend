...
const EditForm = ({navigation} : any) => {
  ...
  const { session, houseUsers, selectedChore } = state;

  useEffect(() => {
    if (selectedChore) {
      setName(selectedChore.name || "");
      setDescription(selectedChore.desc || "");
      setDate(new Date(selectedChore.end * 1000));
      setTime(new Date(selectedChore.end * 1000));
      setSelectedUsers(selectedChore.users);
    }
  },[])
  ...
  async function handleSubmit() {
    ...
    const result = await fetch(
      `${process.env.EXPO_PUBLIC_API_ENDPOINT}/api/v2/chore/${selectedChore?.id}/edit`,
      {
        method: "PATCH",
        headers: {
  ...
  return (
    ...
    >
      ...
      >
        <InputField label="Nome Faccenda" onInputChange={handleNameChange} startingValue={name} />
        <InputField
          label="Descrizione"
      ...
      <View style={{ width: "90%", alignSelf: "center", marginBottom: 20 }}>
        <Button
          icon={<Icon style={{ marginRight: 10 }} name="pencil-outline" type="material-community" color={theme.colors.onBgPrimary} />}
          iconPosition="left"
          title="Modifica Faccenda"
